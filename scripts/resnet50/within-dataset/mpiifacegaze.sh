...
# 用法: bash scripts/resnet50/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        "$PYTHON" main.py --dataset mpiifacegaze --method resnet50 --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            "$PYTHON" main.py --dataset mpiifacegaze --method resnet50 --exp "$exp" --run "$run" "$@"
        fi
    else
        "$PYTHON" main.py --dataset mpiifacegaze --method resnet50 --exp "$exp" --run "$run" "$@"
    fi
...
for fold in $(seq 0 14); do
    ...
    "$PYTHON" main.py --dataset mpiifacegaze --method resnet50 --test --exp "$exp" --run "$(printf 'fold_%02d' "$fold")"
done
