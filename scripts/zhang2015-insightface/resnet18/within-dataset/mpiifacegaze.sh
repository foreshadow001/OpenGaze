#!/usr/bin/env bash
# resnet18 / within-dataset: MPIIFaceGaze leave-one-out（15 折，折 i 以 p{i:02d} 为测试被试）
...
# 用法: bash scripts/zhang2015-insightface/resnet18/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
for d in $(ls exp/ 2>/dev/null | grep -E '^exp[0-9]+$' | sort -rV); do
    if grep -q "^dataset_config: zhang2015-insightface/mpiifacegaze$" "exp/$d/config.yaml" 2>/dev/null \
       && grep -q "^method_config: resnet18$" "exp/$d/config.yaml" 2>/dev/null; then
        exp=$d
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        py main.py --dataset zhang2015-insightface/mpiifacegaze --method resnet18 --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            py main.py --dataset zhang2015-insightface/mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
        fi
    else
        py main.py --dataset zhang2015-insightface/mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
    fi
...
for fold in $(seq 0 14); do
    ...
    "$PYTHON" main.py --dataset zhang2015-insightface/mpiifacegaze --method resnet18 --test --exp "$exp" --run "$(printf 'fold_%02d' "$fold")"
done
