...
# 用法: bash scripts/resnet18/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
for d in $(ls exp/ 2>/dev/null | grep -E '^exp[0-9]+$' | sort -rV); do
    if grep -q "^dataset_config: mpiifacegaze$" "exp/$d/config.yaml" 2>/dev/null \
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        "$PYTHON" main.py --dataset mpiifacegaze --method resnet18 --run "$run" "$@"
        exp=$(latest_exp)
    elif [ -d "exp/$exp/$run" ]; then
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            "$PYTHON" main.py --resume "$exp" --run "$run"          # 中断续训
        else
            rm -rf "exp/$exp/$run"                                   # 空壳重建
            "$PYTHON" main.py --dataset mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
        fi
    else
        "$PYTHON" main.py --dataset mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
    fi
...
for fold in $(seq 0 14); do
    ...
    "$PYTHON" main.py --dataset mpiifacegaze --method resnet18 --test --exp "$exp" --run "$(printf 'fold_%02d' "$fold")"
done
