#!/usr/bin/env bash
# resnet18 / ours-without-roll-ccs / within-dataset: MPIIFaceGaze leave-one-out（标签 = ccs）
...
# 用法: bash scripts/ours-without-roll-ccs/resnet18/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        py main.py --dataset $DS --method resnet18 --label ccs --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            rm -rf "exp/$exp/$run"
            py main.py --dataset $DS --method resnet18 --label ccs --exp "$exp" --run "$run" "$@"
        fi
    else
        py main.py --dataset $DS --method resnet18 --label ccs --exp "$exp" --run "$run" "$@"
    fi
