#!/usr/bin/env bash
# resnet18 / ours-without-roll-hcs / within-dataset: MPIIFaceGaze leave-one-out（标签 = hcs）
...
# 用法: bash scripts/ours-without-roll-hcs/resnet18/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
for d in $(ls exp/ 2>/dev/null | grep -E '^exp[0-9]+$' | sort -rV); do
    if grep -q "^dataset_config: $DS$" "exp/$d/config.yaml" 2>/dev/null \
       && grep -q "^method_config: resnet18$" "exp/$d/config.yaml" 2>/dev/null \
       && grep -q "^  label: hcs$" "exp/$d/config.yaml" 2>/dev/null; then
        exp=$d
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        py main.py --dataset $DS --method resnet18 --label hcs --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            rm -rf "exp/$exp/$run"
            py main.py --dataset $DS --method resnet18 --label hcs --exp "$exp" --run "$run" "$@"
        fi
    else
        py main.py --dataset $DS --method resnet18 --label hcs --exp "$exp" --run "$run" "$@"
    fi
...
for fold in $(seq 0 14); do
    ...
    "$PYTHON" main.py --dataset $DS --method resnet18 --test --exp "$exp" --run "$(printf 'fold_%02d' "$fold")"
done
