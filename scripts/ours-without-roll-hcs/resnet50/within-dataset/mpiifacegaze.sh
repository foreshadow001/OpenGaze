#!/usr/bin/env bash
# resnet50 / ours-without-roll-hcs / within-dataset: MPIIFaceGaze leave-one-out（标签 = hcs）
...
# 用法: bash scripts/ours-without-roll-hcs/resnet50/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        py main.py --dataset $DS --method resnet50 --label hcs --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            rm -rf "exp/$exp/$run"
            py main.py --dataset $DS --method resnet50 --label hcs --exp "$exp" --run "$run" "$@"
        fi
    else
        py main.py --dataset $DS --method resnet50 --label hcs --exp "$exp" --run "$run" "$@"
    fi
