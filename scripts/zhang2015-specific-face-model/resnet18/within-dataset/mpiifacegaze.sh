...
# 用法: bash scripts/zhang2015-specific-face-model/resnet18/within-dataset/mpiifacegaze.sh
set -euo pipefail
...
run_one() {
    ...
    if [ -z "$exp" ]; then
        py main.py --dataset zhang2015-specific-face-model/mpiifacegaze --method resnet18 --run "$run" "$@"
        exp=$(latest_exp)
        ...
        if ls "exp/$exp/$run/ckpt/"*.pth >/dev/null 2>&1; then
            ...
            py main.py --dataset zhang2015-specific-face-model/mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
        fi
    else
        py main.py --dataset zhang2015-specific-face-model/mpiifacegaze --method resnet18 --exp "$exp" --run "$run" "$@"
    fi
...
for fold in $(seq 0 14); do
    ...
    "$PYTHON" main.py --dataset zhang2015-specific-face-model/mpiifacegaze --method resnet18 --test --exp "$exp" --run "$(printf 'fold_%02d' "$fold")"
done
