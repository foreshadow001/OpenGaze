...
5. **统一数据格式**：所有数据集预处理为 h5：`face_patch (N,224,224,3) uint8`、`face_gaze (N,2) (pitch,yaw) 弧度`，目录 `<data_dir>/<sub_folder>/<file>`。ETH-XGaze 官方 h5 为 BGR 存储，仅其 loader 做 BGR→RGB 翻转；其他数据集预处理时直接存 RGB。

6. **数据位置**：`/media/hitsz/ylx/`（xgaze_224、mpiifacegaze_insightface_224、MPIIFaceGaze、GazeCapture、EVE_dataset、Gaze360）。
...
- `preprocess/` 预处理：`common.py`（FailureRecorder 失败样本记录 + 运行目录）；管线目录 `zhang2015-insightface/`（Zhang2015 归一化 + insightface 关键点）：`normalize_xgaze.py`（XGazePreprocessor 类，独立配置 LANDMARK_USE/IDX6/FLIP_CAMERAS/EXCLUDE_CAMERAS，face model 本目录独立加载）、`normalize_mpiifacegaze.py`、`face_model_xgaze.txt`；每次运行留档 `preprocess/log/<dataset>_<时间戳>/{run.log, failures.json}`；失败/跳过帧逐条记录（imread_failed / no_face_detected / no_annotation / error:*）
