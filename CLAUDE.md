...
通用视线估计训练/测试平台（多数据集：ETH-XGaze、MPIIFaceGaze、EVE 已接入；GazeCapture 预处理已就绪、训练侧待接入）。基于 ETH-XGaze 官方代码演化而来。
...
6. **数据位置**（2026-08 大迁移后）：原始数据在 `/media/hitsz/zyx/`（GazeCapture、EVE_dataset、MPIIFaceGaze、Gaze360）与 `/media/hitsz/Expansion/`（xgaze_raw：数据/标注/标定）；**预处理产物统一在 `/media/hitsz/ylx/`**（xgaze / mpiifacegaze / gazecapture / eve 四套 `*_insightface_224`，各带 `landmarks/` 轻量索引）；nvme `/home/hitsz/dataset/` 仅剩 `xgaze_specific_face_model_224`（另有三套 insightface 旧副本待用户清理，勿再写入）。
   ...
   - GazeCapture：原始数据 `/media/hitsz/zyx/GazeCapture`（1474 个五位数 session 目录：`frames/` + 帧级 json，官方代码 /home/hitsz/GazeCapture；2026-08 由 ylx 迁入 zyx，内参 xml 由 generate_calibration.py 在新位置重新生成）；**官方划分为 session 级**（iTracker reference_metadata.mat 提取，已与 info.json 的 Dataset 字段互证），保存在 `configs/splits/gazecapture_sessions.yaml`：train 1321（官方 val 50 已并入，本平台不用验证集）/ test 150 session，excluded 3 个（01185/01730/02065，预处理跳过）；**不使用官方 appleFace/eye bbox 与 IsValid**——人脸定位完全走 insightface 管线，帧有效性由管线成败决定；视线标签在 dotInfo.json 的 XCam/YCam（相机系平面坐标）
   ...
   - EVE：`/home/hitsz/dataset/eve_insightface_224/{train,test}/<trainNN|valNN>.h5`（自预处理 zhang2015-insightface：4 相机 5Hz，train 889,296 / test 108,095 样本，BGR + lzf）；官方 test01–06 无 Tobii 标注弃用 → **平台 test = 官方 val01–05**；h5 三索引（frame/cam/step_index）+ attrs 自描述回溯原始 mp4 帧；单一配置 `configs/datasets/eve.yaml`（与 xgaze 同构，无 LOO）；勘探、几何验证与决策记录在 `preprocess/zhang2015-insightface/eve/dataset_report.md`
...
- `preprocess/` 预处理：`common.py`（FailureRecorder 失败样本记录 + 运行目录）；管线目录 `zhang2015-insightface/`（Zhang2015 归一化 + insightface 关键点）：`normalize_xgaze.py`（XGazePreprocessor 类）、`normalize_mpiifacegaze.py`、`face_model_xgaze.txt`、`gazecapture/`（**已实现已验证**：GazeCapturePreprocessor + 方向映射 docstring；`generate_calibration.py` 生成内参 xml → `/media/hitsz/zyx/GazeCapture/calibration/`（15 设备×双分辨率，cam00.xml 同构）；`calibrate_fx.py` 为 fx 自标定负结果实验；`front_cameras.yaml` 设备分组与 fx；`dot_transfer.md` 四朝向 dot→CCS 定稿公式；`obtain_camera_intrinsics.md` 内参获取全记录）、`eve/`（**已实现已验证**：EVEPreprocessor，4 相机 5Hz，几何链路与公式验证见 `eve/dataset_report.md`，D1–D5 决策已定）、`extract_landmarks.py`（从四套预处理产物抽取 `facial_landmarks_2d`+索引字段 → 各数据集 `<root>/landmarks/`，每人/session 一个轻量 h5，eve 含 steps/cameras attrs）；四个预处理器均支持 **landmarks 模式**——配置 `landmarks_dir` 非空即从 landmarks h5 索引遍历帧集、跳过 insightface 检测、特征点直读，与原始模式输出逐字节一致（gazecapture/eve 已验证；xgaze/mpii 原数据出自旧脚本，重跑差异仅末位浮点噪声 ≤0.035°）；另一管线 `zhang2015-specific-face-model/get_face_model/{xgaze,mpiifacegaze,gazecapture,eve}/`（逐数据集的逐人刚体人脸模型生成；xgaze 已实现——单相机多帧联合 BA，方案文档 `personalized_face_modeling.md`，成果已全量生成于 `/home/hitsz/dataset/xgaze_specific_face_model_224/face_models/`（80 人 768 相机模型，train 3.15px / 留出 5.97px 中位，替代通用模型 26.27px）；其余三个数据集待实现，各目录 README 记录差异要点）；预处理器两种形式：`normalize_<ds>.py` 文件或 `<ds>/__init__.py` 目录包，均暴露 `run(config, recorder)`；每次运行留档 `preprocess/<method>/log/<dataset>_<时间戳>/{run.log, failures.json}`（日志归属各管线目录）；失败/跳过帧逐条记录（imread_failed / no_face_detected / no_annotation / invalid_dot / error:*）
