...
6. **数据位置**（2026-08 大迁移后）：原始数据在 `/media/hitsz/zyx/`（GazeCapture、EVE_dataset、MPIIFaceGaze、Gaze360）与 `/media/hitsz/Expansion/`（xgaze_raw：数据/标注/标定）；**预处理产物统一在 `/media/hitsz/ylx/`**（xgaze / mpiifacegaze / gazecapture / eve 四套 `*_insightface_224`）；**特征点索引在各原始数据集目录的 `landmarks/`**（Expansion/xgaze_raw/data、zyx 各数据集根下，由 `extract_landmarks.py` 从预处理产物抽取）；nvme `/home/hitsz/dataset/` 仅剩 `xgaze_specific_face_model_224`（另有三套 insightface 旧副本待用户清理，勿再写入）。
