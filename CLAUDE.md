...
- `datasets/` 四数据集 loader（xgaze / mpiifacegaze / eve / gazecapture，均已接入训练；`base.py` 的 `label_field_of` 按 `dataset.label` 选 `face_gaze`(ccs)/`face_gaze_hcs`(hcs)，v1 产物仅有 ccs）+ `base.py` 统一 h5 基类与 `make_train_loader`（DDP 时挂 DistributedSampler）+ `__init__.py` 工厂（新增数据集注册到 DATASETS；gazecapture 的 session 列表从 `configs/splits/` 读）。v3 管线 `preprocess/ours-without-roll/`：协议 `normalization_protocol.md`（fixed_forward=True roll-only 归一化；几何全部从 v2 产物 h5 精确恢复——`v3_core.py` 共享链：Kabsch(face_landmarks_3d)+face_mat_norm^T·unit(face_gaze)，无 DLT/PnP），`normalize_{xgaze,eve,gazecapture,mpiifacegaze}.py`，字段同 v2 但 `face_head_pose`→`face_head_elev_azim`（世界系：xgaze=cam00 系、EVE=basler 系、GC/MPII=相机）
