"""从预处理产物抽取面部特征点 + 索引信息 -> <数据集目录>/landmarks/（每人/session 一个 h5）
...
根目录默认取 configs/preprocess/<dataset>.yaml 的 output_dir，可用 --root 覆盖。
"""
import argparse
import os
import sys
...
# 数据集 -> (源 h5 相对根的 glob, 保留字段, 是否复制全部 attrs)
SPECS = {
...
def extract(dataset, root, overwrite=False):
    pattern, fields, with_attrs = SPECS[dataset]
    src_files = sorted(Path(root).glob(pattern))
    if not src_files:
        raise SystemExit(f'{root} 下未找到匹配 {pattern} 的 h5')
    out_dir = Path(root) / 'landmarks'
...
def main():
    parser = argparse.ArgumentParser(description='抽取特征点+索引到 landmarks/')
    parser.add_argument('--dataset', required=True, choices=sorted(SPECS))
    parser.add_argument('--root', default=None,
                        help='数据集根目录（默认 configs/preprocess/<ds>.yaml 的 output_dir）')
    ...
    if root is None:
        ...
        root = cfg['output_dir']
    log.info(f'数据集 {args.dataset} | 根目录 {root}')
    extract(args.dataset, root, overwrite=args.overwrite)
