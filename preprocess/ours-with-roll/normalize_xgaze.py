"""ours-with-roll 版 XGaze 归一化（第三版，2026-08-29）
...
用法：python preprocess.py --dataset xgaze --method ours-with-roll
...
log = get_logger('preprocess.ours_with_roll.xgaze')
...
class OursWithRollXGazePreprocessor:
    ...
    def __init__(self, config):
        ...
        if not self.landmarks_dir:
            raise SystemExit('ours-with-roll 管线要求 landmarks_dir（landmarks 模式）')
    ...
    def run(self, recorder: FailureRecorder):
        ...
        log.info(f'ours-with-roll: {len(subjects)} 受试者, 输出 {self.config.output_dir}')
        total = 0
        t_all = time.time()
